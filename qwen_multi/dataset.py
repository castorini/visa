...
def filter_empty(group):
    return len(group['candidates']) > 0
...
class TrainDataset(Dataset):
    def __init__(self, data_args: DataArguments, trainer = None):
        ...
        self.train_data_fb = load_dataset('MrLight/fineweb-visa-multi-na')['train']
        self.train_data_pub = load_dataset('MrLight/publaynet-visa-multi-na')['train']
        self.test_data_wiki = load_dataset('MrLight/nq-visa-multi-na')['test'].remove_columns(['url', 'short_answer_type', 'long_answer', 'image_size'])
        self.test_data_pub = load_dataset('MrLight/publaynet-visa-multi-na')['test']
        ...
        self.train_data_pub_text = self.train_data_pub.filter(lambda x: x['long_answer_type'] == 'text', num_proc=16, batch_size=1).filter(filter_empty, num_proc=16, batch_size=1).filter(filter_narrow_bbx, num_proc=16, batch_size=1).filter(filter_too_edge_bbx, num_proc=16, batch_size=1).select(range(20000))
        self.train_data_pub_non_text = self.train_data_pub.filter(lambda x: x['long_answer_type'] != 'text', num_proc=16, batch_size=1).filter(filter_empty, num_proc=16, batch_size=1).filter(filter_narrow_bbx, num_proc=16, batch_size=1).filter(filter_too_edge_bbx, num_proc=16, batch_size=1).select(range(20000))
        ...
        if 'wiki' in self.data_args.dataset_name:
            ...
            self.corpus_to_use.append(self.train_data_wiki)
            self.corpus_to_use.append(self.test_data_wiki)
        if 'fb' in self.data_args.dataset_name:
            self.data_to_use.append(self.train_data_fb.filter(filter_empty, num_proc=16, batch_size=1).filter(filter_narrow_bbx, num_proc=16, batch_size=1).filter(filter_too_edge_bbx, num_proc=16, batch_size=1))
            self.corpus_to_use.append(self.train_data_fb)
        if 'pub' in self.data_args.dataset_name:
            ...
            self.corpus_to_use.append(self.train_data_pub)
            self.corpus_to_use.append(self.test_data_pub)
        self.train_data = concatenate_datasets(self.data_to_use)
        self.corpus = concatenate_datasets(self.corpus_to_use)

        self.docid2idx = {}
        for idx, docid in enumerate(tqdm(self.corpus['id'])):
    ...
    def __getitem__(self, item) -> Tuple[str, str, str]:
        ...
        for idx, candidate in enumerate(candidates):
            image = self.corpus[self.docid2idx[candidate]]['image']
            image_size = image.size
            scale = image_size[0] / target_width
            ...
            if idx == pos_idx:
                ...
                crop_x1 = random.randint(0, max(0, int(image_size[0] / 2) - 5))
                crop_y1 = random.randint(0, max(0, int(image_size[1] / 2) - 5))
                crop_x2 = random.randint(min(int(image_size[0] / 2) + 5, image_size[0]), image_size[0])
                crop_y2 = random.randint(min(int(image_size[1] / 2) + 5, image_size[1]), image_size[1])
        ...
        bbx_text = '({}, {}), ({}, {})'.format(*bbx)
        if pos_idx != -1:
            target_text = f'Answer: {short_answer}\nTarget Document: {pos_idx+1}\nBounding Box: {bbx_text}'
        else:
            target_text = f'No Answer'

        return images, question, target_text
