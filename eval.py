...
results = []
answer_results = []
with open(f'{args.model}_{args.setting}_output_ha.jsonl', 'w') as f:
    for num in tqdm(range(len(nq_dev))):
        image = nq_dev[num]['image'] #Image.open(BytesIO(response2.content)).resize((980, 980))
        # convert to rgb
        # image = image.convert("RGB")
        image_size = image.size
        # devide 1.25 to make image smaller
        target_width = 700
        scale = image_size[0] / 700
        image = image.resize((int(image_size[0] / scale), int(image_size[1] /  scale)))
        # image_size = image.size
        question = nq_dev[num]['question']
        bbx = nq_dev[num]['bounding_box']
        bbx = [int( bbx[0] /  scale), int(bbx[1] /  scale), int(bbx[2] /  scale), int(bbx[3] /  scale)]
        answer = nq_dev[num]['short_answer']
        ...
        messages = [
            ...
            },
            {
                ...
                'content': [
                    ...
                    {'type': 'text', 'text': f'Image Size: {image.size}\n'},
        ...
        generated_ids = model.generate(**inputs, max_new_tokens=128, do_sample=False, top_p=None, top_k=None, temperature=0)
        ...
        # print(output_text[0])
        try:
            answer = output_text[0].split("Bounding Box: ")[0].replace('Answer:', '').strip()
            ...
            if "Bounding Box: " in output_text[0]:
                ...
                if bounding_box[0] < 1 and bounding_box[1]:
                    bounding_box[0] = bounding_box[0] * image.size[0]
                    bounding_box[1] = bounding_box[1] * image.size[1]
                    bounding_box[2] = bounding_box[2] * image.size[0]
                    bounding_box[3] = bounding_box[3] * image.size[1]
